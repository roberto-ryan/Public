...
<#
.SYNOPSIS
    This function allows you to execute a script block as the currently logged on user.

.DESCRIPTION
    The Invoke-AsCurrentUser function executes a provided script block as the currently logged on user. It has several optional parameters to control the execution, such as NoWait, UseWindowsPowerShell, UseMicrosoftPowerShell, NonElevatedSession, Visible, CacheToDisk, and CaptureOutput. This function is based on the RunAsUser PowerShell module by KelvinTegelaar.

.PARAMETER ScriptBlock
    The script block to be executed.

.PARAMETER NoWait
    If this switch is provided, the function will not wait for the script block to finish executing.

.PARAMETER UseWindowsPowerShell
    If this switch is provided, the function will use Windows PowerShell for execution.

.PARAMETER UseMicrosoftPowerShell
    If this switch is provided, the function will use Microsoft PowerShell for execution.

.PARAMETER NonElevatedSession
    If this switch is provided, the function will run the script block in a non-elevated session.

.PARAMETER Visible
    If this switch is provided, the function will make the PowerShell window visible during execution.

.PARAMETER CacheToDisk
    If this switch is provided, the function will cache the script block to disk before execution.

.PARAMETER CaptureOutput
    If this switch is provided, the function will capture the output of the script block.

.EXAMPLE
    Invoke-AsCurrentUser -ScriptBlock { Get-Process } -UseWindowsPowerShell -Visible

    This command will execute the Get-Process command as the currently logged on user, using Windows PowerShell, and will make the PowerShell window visible during execution.

.NOTES
    This function is based on the RunAsUser PowerShell module by KelvinTegelaar.
...
function invoke-ascurrentuser {
    ...
    if ($encodedcommand.length -gt $MaxLength -and $CacheToDisk -eq $false) {
        Write-Error -message "$($env:ComputerName) - The encoded script is longer than the command line parameter limit. Please execute the script with the -CacheToDisk option."
        return
    }
    if ($UseMicrosoftPowerShell -and -not (Test-Path -Path "$env:ProgramFiles\PowerShell\7\pwsh.exe")) {
        Write-Host -message "$($env:ComputerName) - Not able to find Microsoft PowerShell v7 (pwsh.exe). Ensure that it is installed on this system"
        return
    }
    ...
    if (-not $privs -or ($privs -band [RunAsUser.PrivilegeAttributes]::Disabled)) {
        Write-Host -message "$($env:ComputerName) - Not running with correct privilege. You must run this script as system or have the SeDelegateSessionUserImpersonatePrivilege token."
        return
    }
    else {
        try {
...
function Install-vtsPwsh {
  msiexec.exe /i "https://github.com/PowerShell/PowerShell/releases/download/v7.4.3/PowerShell-7.4.3-win-x64.msi" /qn
...
.EXAMPLE
PS C:\> Get-vts365DistributionListRecipients -DistributionList "Group1", "Group2"
