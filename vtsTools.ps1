...
<#
.SYNOPSIS
    This function starts a screen recording using FFmpeg.

.DESCRIPTION
    The Start-vtsScreenRecording function starts a screen recording of the current desktop session. 
    It uses FFmpeg to capture the screen at a rate of 5 frames per second and scales the output to a resolution of 1280x720. 
    The output is saved as an MKV file in the "C:\Windows\Temp\VTS\rc" directory. 
    The filename is formatted as "T[timestamp]-[computername]-[username].mkv".

.EXAMPLE
    Start-vtsScreenRecording
...
.NOTES
    ...
    If FFmpeg is not found, it will attempt to download and install it using Chocolatey and aria2.
...
#>
  ...
  invoke-ascurrentuser -scriptblock {
      ...
      $script = {
          & "C:\Windows\Temp\VTS\rc\ffmpeg-master-latest-win64-gpl-shared\bin\ffmpeg.exe" -f gdigrab -framerate 5 -i desktop -vf "scale=1280:720" "C:\Windows\Temp\VTS\rc\T$(Get-Date -f hhmm-MM-dd-yyyy)-$($env:COMPUTERNAME)-$($env:USERNAME).mkv"
      }
...
<#
.SYNOPSIS
  This function stops the screen recording process.

.DESCRIPTION
  The Stop-vtsScreenRecording function checks if there is an ongoing screen recording process (ffmpeg). 
  If found, it kills the process and stops the recording. If no such process is found, it outputs a message stating the same.

.EXAMPLE
  Stop-vtsScreenRecording
  This command stops the ongoing screen recording process.

.LINK
  Utilities
#>
function Stop-vtsScreenRecording {
  if ((Get-Process ffmpeg)) {
      (Get-Process ffmpeg).kill()
      Write-Host "Screen recording stopped."
  }
  else {
      Write-Host "No screen recording process found."
  }
}