  ...
  if ($prevCmdIdx -ne $cmdIdx) { $dirtyCmds = $true; $dirtyDetails = $true; $scrollDetail = 0; $prevCmdIdx = $cmdIdx }
    ...
    if ($dirtyCats -or $needFull) {
      ...
      if ($scrollCat -gt 0 -and $cats.Count -gt 0) {
        ...
        [Console]::BackgroundColor = $bgTop; [Console]::ForegroundColor = [ConsoleColor]::DarkGray; [Console]::Write("^`n|"); [Console]::ResetColor()
      }
      if ( ($scrollCat + $listHeight) -lt $cats.Count ) {
        ...
        [Console]::BackgroundColor = $bgBot; [Console]::ForegroundColor = [ConsoleColor]::DarkGray; [Console]::Write("|`nv"); [Console]::ResetColor()
      }
      $dirtyCats = $false
    }
  ...
  $lineY = $dTop
      if ($detail) {
        ...
        if ($scrollDetail -gt 0) {
          ...
          Write-At $dTop $xInd "^`n|" ([ConsoleColor]::Green)
        }
        if (($scrollDetail + $dHeight) -lt $detailLines.Count) {
          # Down indicator at last visible line
          Write-At ($dTop + $dHeight - 1) $xInd "|`nv" ([ConsoleColor]::Green)
        }
      } else {
