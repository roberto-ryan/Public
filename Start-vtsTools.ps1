  ...
  if ($prevCmdIdx -ne $cmdIdx) { $dirtyCmds = $true; $dirtyDetails = $true; $scrollDetail = 0; $prevCmdIdx = $cmdIdx }
    ...
    if ($dirtyCats -or $needFull) {
      ...
      if ( ($scrollCat + $listHeight) -lt $cats.Count ) {
        ...
        [Console]::BackgroundColor = $bgBot; [Console]::ForegroundColor = [ConsoleColor]::DarkGray; [Console]::Write('|`n˅'); [Console]::ResetColor()
      }
      $dirtyCats = $false
    }
  ...
  $lineY = $dTop
      if ($detail) {
        ...
        if (($scrollDetail + $dHeight) -lt $detailLines.Count) {
          ...
          Write-At ($dTop + $dHeight - 1) $xInd '|`n˅' ([ConsoleColor]::Green)
        }
      } else {
